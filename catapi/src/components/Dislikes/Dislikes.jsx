...
import { fetchAllVotes, fetchAddVote } from "@/api/catapi";
                  ...
                  <div key={idx} className={breedStyle.item}>
                    ...
                    <div className={breedStyle.imgOverlayBox} >
                      {/* <button className={pageStyles.imgOverlayBtn} onClick={(e) => {
                        e.preventDefault();
                        heart = "Changed to like";
                        console.log("Changed value");
                        const filtID = newLogs.filter((old) => old.image_id !== i.image_id)
                        if (filtID) {
                          dispatch(fetchAddVote({ image_id: i?.image_id, value: 1 }))
                        }
